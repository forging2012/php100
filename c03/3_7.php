...
function is_login(){
	...
	foreach($users as $key=>$value){
		//比较SESSION中的值与用户数组中的值
		if($value["username"]==$u and $value["password"]==$p){
			//如果SESSION中的值与用户数组中的值有一对是相等的,返回TRUE
			return true;
		}
	}
...
function login(){
	...
	foreach($users as $key=>$value){
		...
		if($value["username"]==$u and $value["password"]==$p){
			//如果表单提交的变量等于数组中的值,设置SESSION值,供is_login()函数检查
			...
			echo "<script>window.navigate('3_7.php');</script>";
			return true;
		}
	}
